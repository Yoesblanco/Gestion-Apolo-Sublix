...
import { Moon, Sun, Database, Trash2, Save, LogOut, User as UserIcon } from 'lucide-react';
import './Settings.css';
...
export const Settings: React.FC = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    const confirmSave = window.confirm(
      '¿Estás seguro de que deseas actualizar tu información de perfil? Si cambias tu usuario o contraseña, deberás usarlos en tu próximo inicio de sesión.'
    );
    ...
    if (result.success) {
      addToast(result.message || 'Perfil actualizado correctamente', 'success');
      setProfileData((prev) => ({ ...prev, password: '' }));
      setIsEditing(false);
    } else {
      addToast(result.message || 'Error al actualizar perfil', 'error');
    }
  };
  ...
  const handleClearData = () => {
    if (
      window.confirm(
        '¿ESTÁS SEGURO? Esto eliminará TODOS los datos locales (pedidos, clientes, inventario). Esta acción no se puede deshacer.'
      )
    ) {
      localStorage.clear();
          ...
          <div className="card-body">
            <div className="profile-summary">
              <div className="user-profile-header">
                <div className="user-avatar-large">{user?.name?.charAt(0) || 'A'}</div>
