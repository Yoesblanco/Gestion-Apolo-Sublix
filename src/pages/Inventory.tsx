...
export const Inventory: React.FC = () => {
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    const newProduct: Product = {
      ...
      stock: stockNum,
      price: priceNum,
      status: stockNum > 10 ? 'En Stock' : stockNum > 0 ? 'Bajo Stock' : 'Sin Stock',
    };
  ...
  const handleEditProduct = (e: React.FormEvent) => {
    ...
    if (oldName !== newName) {
      setStockHistory((prev) =>
        (prev || []).map((h) =>
          h.productName && h.productName.toLowerCase() === oldName.toLowerCase()
            ? { ...h, productName: newName }
            : h
        )
      );
      setToBuy((prev) =>
  ...
  const handleCleanupHistory = () => {
    if (window.confirm('¿Eliminar registros de productos que ya no existen?')) {
      ...
      setStockHistory((prev) => (prev || []).filter((h) => h.productName && active.includes(h.productName)));
    }
  };
        ...
        <div style={MODAL_OVERLAY_STYLE} onClick={() => setAddModalOpen(false)}>
          <div style={modalContentStyle('480px')} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddProduct}>
              ...
              <div style={MODAL_GROUP}>
                <label style={MODAL_LABEL}>
                  <Hash size={14} /> Stock Inicial (unidades)
                </label>
                <input
                  style={MODAL_INPUT}
                  type="number"
              ...
              <div style={MODAL_GROUP}>
                <label style={MODAL_LABEL}>
                  <Banknote size={14} /> Costo Unitario de Compra ($)
                </label>
                <input
                  style={MODAL_INPUT}
                  type="number"
        ...
        <div style={MODAL_OVERLAY_STYLE} onClick={() => setStockModalOpen(false)}>
          <div style={modalContentStyle('440px')} onClick={(e) => e.stopPropagation()}>
            <ModalHeader
              icon={PlusCircle}
              title="Agregar Stock"
              subtitle={`Producto: ${selectedProduct?.name}`}
            ...
            <form onSubmit={handleUpdateStock}>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span>
                    → Nuevo: <strong style={{ color: 'var(--accent)' }}>{(selectedProduct?.stock || 0) + (parseInt(stockToAdd) || 0)}</strong>
                  </span>
                </div>
                {parseFloat(purchasePrice) > 0 && (
                  <div style={{ borderTop: '1px solid rgba(16,185,129,0.2)', paddingTop: '8px', display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <strong style={{ color: 'var(--danger)' }}>
                      ${formatUSD((parseInt(stockToAdd) || 0) * parseFloat(purchasePrice))}
                    </strong>
                  </div>
                )}
              </div>
        ...
        <div style={MODAL_OVERLAY_STYLE} onClick={() => setEditModalOpen(false)}>
          <div style={modalContentStyle('480px')} onClick={(e) => e.stopPropagation()}>
            <ModalHeader
              ...
              subtitle="Modifica nombre, categoría o precio"
            ...
            <form onSubmit={handleEditProduct}>
              ...
              <div style={MODAL_GROUP}>
                <label style={MODAL_LABEL}>
                  <Banknote size={14} /> Costo Unitario ($)
                </label>
                <input
                  style={MODAL_INPUT}
                  type="number"
                ...
                [...filteredProducts]
                  ...
                  .map((p) => (
                    <tr key={p.id} className={p.stock === 0 ? 'out-of-stock-row' : ''}>
                      ...
                      <td>
                        <span className={`status-badge ${p.status?.toLowerCase().replace(' ', '-') || 'sin-stock'}`}>
                          {p.status || 'Sin Stock'}
                        </span>
                      </td>
                      <td>
                        <div className="inv-actions">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditFormData({ name: p.name, category: p.category || 'Sublimación', price: String(p.price) });
